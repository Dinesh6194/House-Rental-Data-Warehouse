...
create table Agent_Info_Dim as select Distinct(a.Person_Id),p.title||' '||p.first_name||' '||p.last_name as "Agent Name" from monre.Agent a join monre.person p on a.person_id=p.person_id;
...
select a.person_id from agent_tempfact a join agent_office_bridge_dim b on a.person_id =b.person_id where b.office_id in (select office_id from monre.agent_office group by office_id having count(person_id)<4);
update agent_tempfact set office_size='Small' where person_id in (select a.person_id from agent_tempfact a join agent_office_bridge_dim b on a.person_id =b.person_id where b.office_id in (select office_id from monre.agent_office group by office_id having count(person_id)<4));
