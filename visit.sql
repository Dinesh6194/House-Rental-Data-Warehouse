...
create table visit_fact as select client_person_id,property_id,season_id,count(visit_date) "Total number of Visits" from visit_tempfact group by client_person_id,property_id,season_id;

select sum("Total number of Visits") "Total number of Visits",season_id from visit_fact where season_id='Autumn' group by season_id;
