...
drop table rent_time_period_dim;
...
create table rent_property_scd as select rent_start_date, rent_end_date, property_id, price from monre.rent where not rent_start_date is null;

select * from rent_property_scd;

drop table rent_temp_fact;

create table rent_temp_fact as select rent_id, r.property_id,
...
update rent_temp_fact set scale_id='Medium' where property_no_of_bedrooms between 4 and 6;
update rent_temp_fact set scale_id='Large' where property_no_of_bedrooms between 7 and 10;
...
create table rent_fact as select property_id,property_type,years,category_id,scale_id,period_id,count(rent_id) as "Total Number of Rent",sum(price) as "Total Rental Fees" 
from rent_temp_fact group by (property_id,property_type,years,category_id,scale_id,period_id);
...
from rent_fact r
join property_dim p on r.property_id = p.property_id
